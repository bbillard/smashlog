...
export default function PartenairesScreen() {
  ...
  const filtered = useMemo<PlayerWithStats[]>(() => {
    if (!searchQuery.trim()) return partenaires;
    const q = normalizeStr(searchQuery);
    return partenaires.filter((p) => normalizeStr(p.name).includes(q));
  ...
  return (
    <>
      <Stack.Screen options={{ title: "Partenaires" }} />
      <Screen scrollable nativeHeader>
        <View style={styles.titleRow}>
          <Text style={[styles.title, { color: theme.text }]}>Partenaires</Text>
        </View>

        {/* Barre de recherche */}
        <View style={[styles.searchBar, { backgroundColor: theme.surface, borderColor: isSearchActive ? "rgba(0,229,255,0.3)" : theme.border }]}>
          <Ionicons name="search" size={14} color={isSearchActive ? "#00E5FF" : theme.secondaryText} />
          <TextInput
        ...
        {!isLoading && partenaires.length === 0 ? (
          <EmptyState
            ...
            description="Les partenaires apparaissent ici lorsque tu renseignes un partenaire dans tes matchs."
          />
        ) : null}
        ...
        {!isLoading && filtered.length > 0 ? (
          <View style={styles.section}>
            <SectionHeader
              label={isSearchActive ? "Résultats" : "Partenaires"}
            ...
            {isSearchActive ? (
              ...
              <PlayerGrid items={filtered} role="partenaire" onNavigate={navigateToPlayer} />
            )}
          </View>
        ) : null}
        ...
        {!isLoading && isSearchActive && filtered.length === 0 ? (
          <EmptyState
            ...
            description={`Aucun partenaire ne correspond à "${searchQuery}".`}
          />
        ) : null}
      </Screen>
    </>
