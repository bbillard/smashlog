...
interface MatchDraft {
  ...
  adversaire: string;
  adversaire2: string; // 2e adversaire en double/mixte
  partenaire: string;
  sets: SetDraft[]; // tableau dynamique, MAX_SETS slots pré-alloués
  manualSetsCount: number; // sets ajoutés manuellement
  commentaire: string;
}
...
export function MatchesAccordion({
  ...
  const canAddMatch =
    draft.resultat !== null ||
    draft.adversaire.trim() !== "" ||
    draft.adversaire2.trim() !== "" ||
    draft.partenaire.trim() !== "" ||
  ...
  function handleAddMatch() {
    ...
    const adversaireParts = [
      draft.adversaire.trim(),
      draft.mode !== "simple" ? draft.adversaire2.trim() : "",
    ].filter(Boolean);
    ...
    const newMatch: Match = {
      ...
      partenaire:
        draft.mode !== "simple" && draft.partenaire.trim() ? draft.partenaire.trim() : undefined,
    ...
    <View style={[styles.accordionOpen, { backgroundColor: theme.surface }]}>
      ...
      <View style={styles.accordionOpenBody}>
        ...
        {singleMatch && matches.length >= 1 ? null : <View style={styles.addMatchForm}>
            ...
            <View style={styles.chips}>
              {(["simple", "double", "mixte"] as const).map((mode) => (
                <Pressable
                  key={mode}
                  onPress={() =>
                    setDraft((prev) => ({ ...prev, mode, adversaire2: "" }))
                  }
                  style={[
                    styles.chip,
          ...
          {draft.mode !== "simple" ? (
            <View style={styles.formRow}>
              <Text style={[styles.miniLabel, { color: theme.secondaryText }]}>Partenaire</Text>
              <TextInput
                onChangeText={(v) => setDraft((p) => ({ ...p, partenaire: v }))}
          ...
          {/* 4. Adversaire(s) — 1 champ en simple, 2 en double/mixte */}
          <View style={styles.formRow}>
            <Text style={[styles.miniLabel, { color: theme.secondaryText }]}>
              {draft.mode !== "simple" ? "Adversaires" : "Adversaire"}
            </Text>
            <TextInput
              onChangeText={(v) => setDraft((p) => ({ ...p, adversaire: v }))}
              placeholder="Prénom ou pseudo..."
              placeholderTextColor={theme.secondaryText}
              style={[
                styles.miniInput,
              ...
              value={draft.adversaire}
            />
            {draft.mode !== "simple" ? (
              <TextInput
                onChangeText={(v) => setDraft((p) => ({ ...p, adversaire2: v }))}
                placeholder="2e adversaire..."
                placeholderTextColor={theme.secondaryText}
                style={[
                  styles.miniInput,
                  styles.miniInputSecond,
                  { backgroundColor: theme.surface, borderColor: "rgba(255,255,255,0.1)", color: theme.text },
                ]}
                value={draft.adversaire2}
              />
            ) : null}
          </View>
          ...
          >
            ...
            />
            <Text
              style={[styles.addMatchBtnText, !canAddMatch && styles.addMatchBtnTextDisabled]}
            >
              Ajouter ce match
            </Text>
          </Pressable>
